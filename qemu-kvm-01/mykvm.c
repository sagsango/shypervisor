...
/*
 * mykvm.c - educational VT-x single-step instruction experiment harness
 *
 * WARNING:
 * - not production-safe
 * - one global VM / one global vCPU
 * - simplified register handling
 * - simplified VMCS guest/host state
 * - intended as an experiment scaffold
 */
...
/* Today:KVM only supports 1 vm with 1 CPU
 * Future: multiple vm with multiple CPUs
 * TODO: check how KVM does that
 *	 when there there multiple qemu-vm
 *	 kvm has to store all the metadata
 *	 somewhere
 */
struct mykvm_state {
    struct mutex lock;
    struct mykvm_vm vm; /* vm = virtual memory not virtual machine? */
    struct mykvm_vcpu vcpu;
    struct mykvm_run last_run;
};
...
static inline int my_vmlaunch(void)
{
    u8 ret;
    asm volatile("vmlaunch; setna %0"
                 : "=rm"(ret)
                 :
                 : "cc", "memory");
    return ret ? -EIO : 0;
...
static inline int my_vmresume(void)
{
    u8 ret;
    asm volatile("vmresume; setna %0"
                 : "=rm"(ret)
                 :
                 : "cc", "memory");
    return ret ? -EIO : 0;
...
static int mykvm_build_ept_1g(struct mykvm_vm *vm)
{
     ...
     * This assumes a simple 2 MiB chunk mapping strategy over the 1 GiB guest RAM.
     * It is enough for experimentation, but not robust for production.
     */
    for (i = 0; i < 512; i++) {
...
static int mykvm_alloc_vcpu_resources(struct mykvm_vcpu *vcpu)
{
    ...
    *(u32 *)vcpu->vmcs_region = rev;
    ...
    vcpu->host_stack = (void *)__get_free_pages(GFP_KERNEL | __GFP_ZERO, 1);
    if (!vcpu->host_stack)
        return -ENOMEM;
    vcpu->host_stack_top = (unsigned long)vcpu->host_stack + (2 * PAGE_SIZE);
...
{
    if (vcpu->host_stack)
        free_pages((unsigned long)vcpu->host_stack, 1);
    if (vcpu->vmcs_region)
...
extern void mykvm_vmexit_stub(void);
void mykvm_vmexit_handler(void);

asm(
".global mykvm_vmexit_stub\n"
"mykvm_vmexit_stub:\n"
"    push %rax\n"
"    push %rbx\n"
"    push %rcx\n"
...
void mykvm_vmexit_handler(void)
{
    u64 exit_reason = 0, qual = 0, guest_rip = 0;
    struct mykvm_vcpu *vcpu = &g.vcpu;

    memset(&g.last_run, 0, sizeof(g.last_run));

    my_vmread(VMCS_RO_EXIT_REASON, &exit_reason);
    my_vmread(VMCS_RO_EXIT_QUALIFICATION, &qual);
    my_vmread(VMCS_GUEST_RIP, &guest_rip);

    switch ((u32)(exit_reason & 0xffff)) {
    case EXIT_REASON_MONITOR_TRAP_FLAG:
        g.last_run.exit_reason = MYKVM_EXIT_SINGLE_STEP;
        g.last_run.single_step.guest_rip = guest_rip;
        vcpu->regs.rip = guest_rip;
        break;

    case EXIT_REASON_CPUID:
        g.last_run.exit_reason = MYKVM_EXIT_CPUID;
        g.last_run.cpuid.guest_rip = guest_rip;
        vcpu->regs.rip = guest_rip;
        break;

    case EXIT_REASON_IO_INSTRUCTION:
        g.last_run.exit_reason = MYKVM_EXIT_IO;
        g.last_run.io.port = (qual >> 16) & 0xffff;
        g.last_run.io.direction = (qual >> 3) & 1;
        g.last_run.io.size = (qual & 0x7) + 1;
        g.last_run.io.count = 1;
        g.last_run.io.data = vcpu->regs.rax & 0xff;
        g.last_run.io.guest_rip = guest_rip;
        vcpu->regs.rip = guest_rip;
        break;

    case EXIT_REASON_HLT:
        g.last_run.exit_reason = MYKVM_EXIT_HLT;
        g.last_run.hlt.guest_rip = guest_rip;
        vcpu->regs.rip = guest_rip;
        break;

    case EXIT_REASON_EPT_VIOLATION: {
        u64 gla = 0, gpa = 0;
        my_vmread(VMCS_RO_GUEST_LINEAR_ADDR, &gla);
        my_vmread(VMCS_RO_GUEST_PHYSICAL_ADDR, &gpa);
        g.last_run.exit_reason = MYKVM_EXIT_EPT_VIOLATION;
        g.last_run.ept.qualification = qual;
        g.last_run.ept.guest_linear_address = gla;
        g.last_run.ept.guest_physical_address = gpa;
        g.last_run.ept.guest_rip = guest_rip;
        vcpu->regs.rip = guest_rip;
        break;
    }

    default:
        g.last_run.exit_reason = MYKVM_EXIT_INTERNAL_ERROR;
        g.last_run.internal.guest_rip = guest_rip;
        g.last_run.internal.raw_exit_reason = exit_reason;
        g.last_run.internal.qualification = qual;
        vcpu->regs.rip = guest_rip;
        break;
    }
    /* TODO: We should notify the qemu after this? 
     *       But our experimentation is just to check
     *       if the instruction is creating vm exit or not so
     *       this should be fine
     */
}
...
static int mykvm_setup_vmcs(struct mykvm_vm *vm, struct mykvm_vcpu *vcpu)
{
    ...
    my_vmwrite(VMCS_HOST_RSP, vcpu->host_stack_top);
    my_vmwrite(VMCS_HOST_RIP, (unsigned long)mykvm_vmexit_stub);
    my_vmwrite(VMCS_HOST_SYSENTER_CS, 0);
    my_vmwrite(VMCS_HOST_SYSENTER_ESP, 0);
    my_vmwrite(VMCS_HOST_SYSENTER_EIP, 0);

    /* guest state - simplified */
...
static int mykvm_prepare_vcpu(struct mykvm_vcpu *vcpu)
{
    ...
    vcpu->cpu = get_cpu();
    /* XXX: n the Linux kernel, smp_call_function_single is a core utility
     *	    used to execute a specific function on a targeted CPU. It is a
     *	    primary mechanism for Inter-Processor Interrupts (IPIs), allowing
     *	    one processor to request service from another
     */
...
/* XXX: Install the test instruction on the vm
 *	We can set the instruction by ioctl
 *	MYKVM_SET_TEST
 *
 *
 *	We can set 2 instruction insstead,
 *	1st which we want to test
 *	and 2nd standard instruction, to resuest vm_exit
 */
...
/* XXX: MYKVM_RUN ioctl handler
 *	Seems like during the VM_EXIT
 *	it will get returned to usespace my-qemu
 *	so qemu can handle the exit
 */
static int mykvm_run_vcpu(struct mykvm_vcpu *vcpu, struct mykvm_run *run)
{
    int ret;
    u64 err = 0, guest_rip = 0, exit_reason = 0, qual = 0;
    ...
    if (ret) {
        ...
        run->exit_reason = MYKVM_EXIT_FAIL_ENTRY;
        run->fail_entry.guest_rip = guest_rip;
        run->fail_entry.instruction_error = (u32)err;
        run->fail_entry.raw_exit_reason = (u32)exit_reason;
        run->fail_entry.qualification = qual;
        return 0;
    }

    memcpy(run, &g.last_run, sizeof(*run));
    return 0;
}
...
MODULE_DESCRIPTION("Educational VT-x single-step hypervisor harness");
