...
int main(void)
{
    ...
    /* XXX: We will add the instructions of the x86 here
     *	    and will test one by one on the same vm.
     */
    ...
    /* XXX: Run all the test instruction one by one */
    for (size_t i = 0; i < nr_tests; i++) {
        ...
        case MYKVM_EXIT_SINGLE_STEP:
            printf("result: instruction executed in guest, then single-step VM-exit\n");
            printf("guest_rip(after)=0x%llx\n",
                   (unsigned long long)run.single_step.guest_rip);
            break;

        case MYKVM_EXIT_CPUID:
            printf("result: CPUID caused VM-exit first\n");
            printf("guest_rip=0x%llx\n",
                   (unsigned long long)run.cpuid.guest_rip);
            break;

        case MYKVM_EXIT_IO:
            printf("result: IO instruction caused VM-exit first\n");
            printf("port=0x%x dir=%u size=%u data=0x%llx guest_rip=0x%llx\n",
                   run.io.port,
                   run.io.direction,
                   run.io.size,
                   (unsigned long long)run.io.data,
                   (unsigned long long)run.io.guest_rip);
            break;

        case MYKVM_EXIT_HLT:
            printf("result: HLT caused VM-exit first\n");
            printf("guest_rip=0x%llx\n",
                   (unsigned long long)run.hlt.guest_rip);
            break;

        case MYKVM_EXIT_EPT_VIOLATION:
            printf("result: EPT violation\n");
            printf("qualification=0x%llx gpa=0x%llx gla=0x%llx rip=0x%llx\n",
                   (unsigned long long)run.ept.qualification,
                   (unsigned long long)run.ept.guest_physical_address,
                   (unsigned long long)run.ept.guest_linear_address,
                   (unsigned long long)run.ept.guest_rip);
            break;
        ...
        default:
            printf("result: unknown exit reason %u\n", run.exit_reason);
            break;
        }
